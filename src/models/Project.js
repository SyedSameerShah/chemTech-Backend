...
const versionSchema = new mongoose.Schema({
  ...
  status: {
    type: String,
    enum: ['Draft', 'In Progress', 'Pending Approval', 'Approved', 'Rejected'],
    default: 'In Progress'
  },
  createdOn: {
    type: Date,
...
projectSchema.statics.generateProjectId = function(industryType, plantType) {
  const industry = industryType.replace(/[^A-Z]/g, '').substring(0, 4).toUpperCase();
  const plant = plantType.replace(/[^A-Z]/g, '').substring(0, 4).toUpperCase();
