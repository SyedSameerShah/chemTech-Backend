...
const createProject = async (req, res) => {
  try {
    ...
    const projectId = ProjectModel.generateProjectId(data.industryType, data.plantType);
    const projectName = data.projectName || ProjectModel.generateProjectName(data.industryType, data.plantType);
      ...
      },
      {
        $project: {
          ...
          statusCounts: {
            $reduce: {
              input: '$byStatus',
              initialValue: { Draft: 0, Submitted: 0, Approved: 0, Rejected: 0 },
              in: {
                Draft: {
                  $cond: [{ $eq: ['$$this', 'Draft'] }, { $add: ['$$value.Draft', 1] }, '$$value.Draft']
                },
                Submitted: {
                  $cond: [{ $eq: ['$$this', 'Submitted'] }, { $add: ['$$value.Submitted', 1] }, '$$value.Submitted']
                },
                Approved: {
                  $cond: [{ $eq: ['$$this', 'Approved'] }, { $add: ['$$value.Approved', 1] }, '$$value.Approved']
                },
                Rejected: {
                  $cond: [{ $eq: ['$$this', 'Rejected'] }, { $add: ['$$value.Rejected', 1] }, '$$value.Rejected']
                }
              }
            }
          }
