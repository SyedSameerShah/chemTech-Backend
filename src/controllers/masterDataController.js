...
const getAll = async (req, res) => {
  try {
    ...
    if (Object.keys(query).length === 0 && !search) {
      ...
      const cachedData = await masterDataCache.get(req.tenantId, collectionName, cacheKey);
      if (cachedData) {
    ...
    if (!items) {
      ...
      if (Object.keys(query).length === 0 && !search) {
        await masterDataCache.set(req.tenantId, collectionName, { items, total }, cacheKey);
      }
    }
