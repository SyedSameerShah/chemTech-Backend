...
class TenantConnectionManager {
  ...
  async getConnection(tenantId) {
    ...
    if (this.connections.has(tenantId)) {
      ...
      if (connection.readyState === 1) {
        ...
        logger.warn(
          `Connection in bad state for tenant ${tenantId}, creating new one`
        );
  ...
  async createConnection(tenantId) {
    try {
      ...
      logger.info(
        `Creating new connection for tenant: ${tenantId}, database: ${dbName}`
      );
      ...
      throw new Error(
        `Failed to establish database connection for tenant ${tenantId}`
      );
    }
  }
