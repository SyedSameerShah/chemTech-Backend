...
// Currency schemas
const currencyCodeSchema = z.string().length(3).regex(/^[A-Z]{3}$/, 'Currency code must be 3 uppercase letters');
...
const createProjectSchema = z.object({
  ...
  baseCurrency: z.enum(['INR', 'USD', 'EUR', 'GBP', 'JPY', 'CNY', 'AED', 'SGD']).default('INR'),
...
const bulkCreateEquipmentCostSchema = z.object({
  projectId: z.string().min(1),
  versionId: z.string().min(1),
  items: z.array(createEquipmentCostSchema.omit({ projectId: true, versionId: true })).min(1).max(100)
});
...
const updateVersionStatusSchema = z.object({
  status: z.enum(['Draft', 'In Progress', 'Pending Approval', 'Approved', 'Rejected']),
  notes: z.string().max(1000).optional()
});
